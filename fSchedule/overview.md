...
16. [x] `动态更新计划`：支持客户端更新下次执行计划时间。
17. [x] `任务集群模式`：同一个任务执行将只调度到其中一个客户端执行。
18. [ ] `任务广播模式`：同一个任务将调度到所有可用的客户端上执行。
19. [ ] `数据分片`：任务可根据当前客户端数量自动分片到所有客户端，每个客户端只处理一部份数据，做到并行处理。
20. [ ] `任务依赖`：可以设置任务执行前、执行后时运行依赖的任务。
21. [ ] `分布式日志`：支持日志上传到集群，统一查看。
22. [ ] `任务报表`：统计任务执行结果，支持按分钟、小时、天、月查看。
23. [x] `任务告警`：当任务执行失败、没有可用客户端、调度失败时，发出告警通知。

> 未打勾的，在将来的版本中支持。

## 5、历史回顾
1. `2026-01-30` 发布2.5版本
